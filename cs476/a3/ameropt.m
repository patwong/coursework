function optval = ameropt(sigma, r, T, K, dt, S, method,type,dnorm)
...
if strcmp(method,'CNR')
    for i = 1:2
        ...
        if i ~= 1
            tol = (poff-optval)./poff;
%             size(large)
%             size(tol)
%             tol = abs(max(tol));
            tol = abs(tol);         %set breakpoints
            large = 1/tol;          %when calculating this, it is infinity
        end
        pvect = optval - poff;
        pvect = large.*(pvect < 0);
        P = diag(pvect);
    ...
    while z ~= 1 || t >= T
        %variable time step setup - orig
%         denom = abs(stored_vs(:,2) - stored_vs(:,1));
%         stored_vs(:,2)- stored_vs(:,1)
%         numer = dnorm*max(D,abs(stored_vs(:,2)),abs(stored_vs(:,1)));
%         dt = min(numer*denom)*dt;
        ...
        pvect = large*(pvect < 0);
        zs = zeros(lenS,1);
        P = spdiags([zs.' pvect zs.'],-1:1,lenS,S);
        M = M+P;
